...
if results:
    ...
    rdf.to_csv(f"{output_dir}/final_results.csv", index=False)
    print(f"💾 Saved: {output_dir}/final_results.csv")
    ...
    test_ma = [float(r["Test MA"].strip('%')) for r in results]
    forget_ma = [float(r["Forget MA"].strip('%')) for r in results]

    x = range(len(langs))
    width = 0.35

    # Plot 1: Memorization Accuracy
    axes[0].bar([i - width/2 for i in x], test_ma, width, label='Test MA', alpha=0.8, color='steelblue')
    axes[0].bar([i + width/2 for i in x], forget_ma, width, label='Forget MA', alpha=0.8, color='coral')
    axes[0].axhline(y=30, color='red', linestyle='--', linewidth=1.5, label='Target (30%)')
    axes[0].axhline(y=25, color='red', linestyle=':', linewidth=1.0, alpha=0.5)
    axes[0].set_xlabel('Language', fontsize=11)
    axes[0].set_ylabel('Memorization Accuracy (%)', fontsize=11)
    axes[0].set_title('MA by Language (Hinglish Code-Mixed)', fontsize=12, fontweight='bold')
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(langs, fontsize=10)
    axes[0].legend(fontsize=10)
    axes[0].grid(axis='y', alpha=0.3)
    axes[0].set_ylim(0, 50)

    # Highlight Hinglish (code-mixed)
    hinglish_idx = langs.index('HINGLISH') if 'HINGLISH' in langs else None
    if hinglish_idx is not None:
        axes[0].axvspan(hinglish_idx - 0.5, hinglish_idx + 0.5, alpha=0.1, color='green')
        axes[0].text(hinglish_idx, 45, 'Code-Mixed', ha='center', fontsize=9, style='italic')
    ...
    for r in results:
        try:
            tppl = float(r["Test PPL"]) if r["Test PPL"] != "N/A" else 0
            fppl = float(r["Forget PPL"]) if r["Forget PPL"] != "N/A" else 0
            test_ppl.append(tppl)
            forget_ppl.append(fppl)
        except:
            test_ppl.append(0)
            forget_ppl.append(0)
    
    if any(test_ppl) and any(forget_ppl):
        axes[1].bar([i - width/2 for i in x], test_ppl, width, label='Test PPL', alpha=0.8, color='steelblue')
        axes[1].bar([i + width/2 for i in x], forget_ppl, width, label='Forget PPL', alpha=0.8, color='coral')
        axes[1].set_xlabel('Language', fontsize=11)
        axes[1].set_ylabel('Perplexity', fontsize=11)
        axes[1].set_title('Perplexity by Language', fontsize=12, fontweight='bold')
        axes[1].set_xticks(x)
        axes[1].set_xticklabels(langs, fontsize=10)
        axes[1].legend(fontsize=10)
        axes[1].grid(axis='y', alpha=0.3)

        if hinglish_idx is not None:
            axes[1].axvspan(hinglish_idx - 0.5, hinglish_idx + 0.5, alpha=0.1, color='green')

    plt.tight_layout()
    
    # Save figure
    plot_path = f"{output_dir}/results_plot.png"
